...
def adaptation(modelName, attribute, batch_size=32, plain = 'plain' , pc = 0, speaker = 0, random_seed = 0, gadi = None):
    num_epochs=100
    ...
    # Save path
    savePath = f"/scratch/wa66/dr2845/models/trainedModels/adaptation/individual/{plain}/{pc}pc/{attribute}/bestDevLoss/{modelName}_{learning_rate}_{batch_size}.pth"
    saveAdapPath = f"/scratch/wa66/dr2845/models/trainedModels/adaptation/individual/{plain}/{pc}pc/{attribute}/bestAdapLoss/{modelName}_{learning_rate}_{batch_size}.pth"

    # Load the datasets in GADI
    adaptation_data = myDataset(f"/scratch/wa66/dr2845/models/database/individual/{plain}/{pc}pc/adaptation{speaker}Data.feather", "adaptation",scaler=scaler)
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(test_loader):
            ...
            test_loss += loss.item()
        test_loss /= len(test_loader)
    print(f"Best development test loss: {test_loss:.4f}, CCC = {(1-test_loss):.4f}")
    CCC1 = 1-test_loss
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(test_loader):
            ...
            test_loss += loss.item()
        test_loss /= len(test_loader)
    print(f"Best adaptation test loss: {test_loss:.4f}, CCC = {(1-test_loss):.4f}\n")
    CCC2 = 1-test_loss

    cccs = [CCC1,CCC2]
    np.savetxt(f'/scratch/wa66/dr2845/models/adaptation/individual/results/{plain}/{pc}pc/{attribute}/{modelName}_ccc.txt', cccs)
